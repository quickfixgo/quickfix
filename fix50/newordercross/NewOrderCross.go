...
//New returns an initialized NewOrderCross instance
func New(crossid string, crosstype int, crossprioritization int, sidecrossordmodgrp sidecrossordmodgrp.SideCrossOrdModGrp, instrument instrument.Instrument, transacttime time.Time, ordtype string) *Message {
	var m Message
	m.SetCrossID(crossid)
	m.SetCrossType(crosstype)
	m.SetCrossPrioritization(crossprioritization)
	m.SetSideCrossOrdModGrp(sidecrossordmodgrp)
...
func Route(router RouteOut) (string, string, quickfix.MessageRoute) {
	...
	return enum.ApplVerID_FIX50, "s", r
}
