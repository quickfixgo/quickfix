...
func New(allocid string, alloctranstype string, alloctype int, allocnoorderstype int, side string, instrument instrument.Instrument, quantity float64, avgpx float64, tradedate string) *Message {
	var m Message
	m.SetAllocID(allocid)
	m.SetAllocTransType(alloctranstype)
	m.SetAllocType(alloctype)
	m.SetAllocNoOrdersType(allocnoorderstype)
	m.SetSide(side)
